...
import type { RulesetPlugin, RulesetPluginIndex } from '../engine/types';
  ...
  .action(async (options: RunCommandOptions) => {
    try {
      ...
      if (!message && Array.isArray(err.errors) && err.errors.length > 0) {
        message = err.errors
          .map(inner => (inner && typeof inner === 'object' && 'message' in inner ? String((inner as any).message) : String(inner)))
          .join('; ');
      }
...
async function resolveIndexPath(options: RunCommandOptions): Promise<string> {
...
const extractPluginIndex = (mod: any, sourcePath: string): RulesetPluginIndex => {
  const maybeIndex =
    (mod && typeof mod === 'object' && mod.default && typeof mod.default === 'object' && mod.default) ||
    (mod && typeof mod === 'object' && mod.rulesets && typeof mod.rulesets === 'object' && mod.rulesets);

  if (!maybeIndex) {
    throw new Error(
      `Index '${sourcePath}' levert geen bruikbare export. Verwacht default export { "<rule>": plugin } of named export { rulesets: {...} }.`
    );
  }

  const normalized: RulesetPluginIndex = {};

  for (const [key, value] of Object.entries(maybeIndex)) {
    if (value && typeof value === 'object') {
      const plugin = value as RulesetPlugin;
      normalized[key] = {
        ...plugin,
        id: plugin.id ?? key,
      };
...
async function readJsonInput(specifier: string): Promise<unknown> {
...
async function invokeEngine(document: unknown, plugin: RulesetPlugin, options: RunCommandOptions) {
